    ...
    showItems = function(obj){
        ...
        $(items).each(function(index, element){
            ...
            table_items +="<td class='text-right'>"+element.quantity+"</td>";
                          //  var master =  element.product_package.filter(function(obj) {
                           //    if (obj.id == element.product_package_id) return obj;
                          //  });
                          console.log(element.product_package);
            //table_items +="<td>"+element.product_package+"</td>";
        ...
        if(coupon){
         coupon_amount = (coupon.coupon_in == 1 ? ((total.toFixed(2) /100) *coupon.coupon_value.toFixed(2)) : coupon.coupon_value.toFixed(2)).toFixed(2);
